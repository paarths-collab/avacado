...
export default function PriceCard({ data, onClick }: PriceCardProps) {
  ...
  return (
    ...
    >
      <Card
        onClick={onClick}
        className="group cursor-pointer overflow-hidden border border-foreground/10 bg-card shadow-none hover:border-foreground/20 transition-all duration-300 rounded-leaf relative h-full"
      >
        <AnimatePresence>
          {isTickUp && (
            <motion.div 
